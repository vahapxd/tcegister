...
let embed = new MessageEmbed().setColor("RANDOM").setAuthor(member.guild.name, member.guild.iconURL({dynamic: true})).setFooter("Tc Family ❤️ " + member.guild.name).setTimestamp()
if(security === "şüpheli") {
 ...
 channel.send(`
\`>\` Ailemize hoşgeldin ${member}! Seninle birlikte ${member.guild.memberCount} kişiye ulaştık.
...
\`>\` Sol taraftaki \`<#971483321165049926>\` kanallarından birine girip \`isim-yaş\` belirterek kayıt olabilirsin.`)
};
});
